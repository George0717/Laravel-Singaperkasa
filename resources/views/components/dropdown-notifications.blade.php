@props([
    'align' => 'right',
    'notifications' => []  // Properti untuk menampung notifikasi yang akan ditampilkan
])
...
<div class="relative inline-flex" x-data="{ open: false }">
    ...
    >
        ...
        <div class="absolute top-0 right-0 w-2.5 h-2.5 bg-red-500 border-2 border-gray-100 rounded-full"></div>
    </button>
    <div
        class="origin-top-right z-10 absolute top-full -mr-48 sm:mr-0 min-w-80 bg-white border border-gray-200 py-1.5 rounded-lg shadow-lg overflow-hidden mt-1 {{$align === 'right' ? 'right-0' : 'left-0'}}"                
            ...
            @foreach($notifications as $notification)
                <li class="border-b border-gray-200 last:border-0">
                    <a class="block py-2 px-4 hover:bg-gray-50" href="#0" @click="open = false">
                        <span class="block text-sm mb-2">📣 <span class="font-medium text-gray-800">{{ $notification['title'] }}</span> {{ $notification['message'] }}</span>
                        <span class="block text-xs font-medium text-gray-400">{{ $notification['date'] }}</span>
                    </a>
                </li>
            @endforeach
        </ul>                
