...
<div class="container mx-auto px-4">
    ...
    <a href="{{ route('suratJalan.create') }}" class="btn btn-primary mb-4" onclick="konfirmasiBuat(event)">Buat Surat Jalan Baru</a>
    ...
    <div class="mb-4">
        <form method="GET" action="{{ route('suratJalan.index') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                ...
                <input type="text" name="customer_name" id="cari-customer" class="form-input mt-1 block w-full border-gray-300 rounded-md" placeholder="Cari berdasarkan Nama Customer" value="{{ request('customer_name') }}">
            </div>
            <div>
                <label for="cari-tanggal" class="block text-sm font-medium text-gray-700">Tanggal Pengiriman</label>
                <input type="date" name="delivery_date" id="cari-tanggal" class="form-input mt-1 block w-full border-gray-300 rounded-md" placeholder="Cari berdasarkan Tanggal Pengiriman" value="{{ request('delivery_date') }}">
            </div>
            ...
            <tbody class="bg-white divide-y divide-gray-200" id="tabel-surat-jalan">
                ...
                <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <a href="{{ route('suratJalan.show', $suratJalan) }}" class="text-blue-600 hover:text-blue-900 transition duration-300 ease-in-out">Lihat</a>
                        <a href="{{ route('suratJalan.edit', $suratJalan) }}" class="text-green-600 hover:text-green-900 transition duration-300 ease-in-out ml-4" onclick="konfirmasiEdit(event, {{ $suratJalan->id }})">Edit</a>
                        <form action="{{ route('suratJalan.destroy', $suratJalan) }}" method="POST" class="inline" id="form-hapus-{{ $suratJalan->id }}">
                            @csrf
                            @method('DELETE')
                            <button type="button" onclick="konfirmasiHapus({{ $suratJalan->id }})" class="text-red-600 hover:text-red-900 transition duration-300 ease-in-out ml-4">Hapus</button>
                        </form>
                    </td>
                </tr>
                @empty
